iconst querystring = require('node:querystring');
...
for (const key in push_config) {
  const v = process.env[key] || process.env[`NOTIFY_${key}`];
  if (v) {
    push_config[key] = v;
  }
}
    ...
    if (BARK_PUSH) {
      ...
      const options = {
        ...
        json: {
          ...
          sound: BARK_SOUND,
          group: BARK_GROUP,
          isArchive: BARK_ARCHIVE ? (BARK_ARCHIVE === '1' || BARK_ARCHIVE === 1 || BARK_ARCHIVE === true ? 1 : 0) : undefined,
          level: BARK_LEVEL,
          url: BARK_URL,
          ...params,
        },
